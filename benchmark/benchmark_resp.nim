...
const demoPath{.strdefine.} = "benchmark" / "simple_resp.nim"
...
proc proj(){.thread.} =
  ...
  let bin = startProcess(fmt"nim c -d:port={port} {demoPath}",
...
proc root(){.thread.} =
  acquire(L)
  let test = startProcess(fmt"wrk -t{threadsNum} -c{connections} -d{seconds}s http://127.0.0.1:{port}/plaintext",
      options = testOptions)
...
proc pa(){.thread.} =
  acquire(L)
  let test2 = startProcess(fmt"wrk -t{threadsNum} -c{connections} -d{seconds}s http://127.0.0.1:{port}/json",
      options = testOptions)
